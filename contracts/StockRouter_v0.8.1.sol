...
// ============================================================
// StockRouter v0.8.1 — Idempotensi + Backward-compat Event
// ============================================================
// Fixes from audit:
//   1. Pindahkan logika idempotensi ke dalam fungsi
//   2. Validasi amount > 0 SEBELUM flag di-set
//   3. Validasi tokenConfig[token].supported
//   4. Event backward-compatibility: InventoryRestocked (lama) + InventoryRestockedV2 (baru)
//   5. Dokumentasikan kenapa whenNotPaused tidak dipakai di restock
// ============================================================
...
contract StockRouter is Ownable, ReentrancyGuard {
    ...
    // === IDEMPOTENSI (v0.8.1) ===
    mapping(bytes32 => bool) public executedRestock;

    // ---------- Events ----------
    // Event lama (backward-compatible)
    event InventoryRestocked(address indexed token, uint256 amount);
    // Event baru (dengan requestId)
    event InventoryRestockedV2(address indexed token, uint256 amount, bytes32 indexed requestId);
    ...
    // === RESTOCK (v0.8.1: idempotensi + backward-compat event) ===
    /// @notice Restock inventory dengan proteksi duplikasi via requestId.
    /// @dev Restock tetap bisa dilakukan saat paused (by design).
    ///      Ini memungkinkan keeper mengisi inventory selama emergency.
    /// @param requestId ID unik yang mencegah duplikasi jika transaksi di-retry.
    function restockInventory(address token, uint256 amount, bytes32 requestId)
        external onlyKeeperOrOwner
    {
        require(amount > 0, "zero amount");
        require(tokenConfig[token].supported, "unsupported token");
        require(!executedRestock[requestId], "duplicate restock");

        executedRestock[requestId] = true;
        ...
        // Emit event lama (backward-compatible) + event baru
        emit InventoryRestocked(token, amount);
        emit InventoryRestockedV2(token, amount, requestId);
    }
